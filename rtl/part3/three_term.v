...
//          s3  subtract, round, saturate, register on the AXIS-like output
...
module three_term #(
    ...
    // -------------------------------------------------------------------------
    // Stage 3 : subtract, round-to-nearest, saturate to W bits
    // -------------------------------------------------------------------------
    localparam signed [W+17:0] ROUND_BIAS = (1 <<< 14);  // round-to-nearest

    function signed [W-1:0] sat_w;
        input signed [W+17:0] x;
        reg   signed [W+17:0] x_shr;
        reg   signed [W-1:0] max_pos;
        reg   signed [W-1:0] max_neg;
        begin
            x_shr   = (x + ROUND_BIAS) >>> 15;
            max_pos = {1'b0, {(W-1){1'b1}}};
            max_neg = {1'b1, {(W-1){1'b0}}};
            if (x_shr > $signed({{18{max_pos[W-1]}}, max_pos}))
                sat_w = max_pos;
            else if (x_shr < $signed({{18{max_neg[W-1]}}, max_neg}))
                sat_w = max_neg;
            else
                sat_w = x_shr[W-1:0];
        end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            xk_last_out  <= last_s2;
            xk_index_out <= m_s2;
            xk_re_out    <= sat_w(diff_re);
            xk_im_out    <= sat_w(diff_im);
        end
    end
